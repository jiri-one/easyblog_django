...
class Command(BaseCommand):
    ...
    def update_django_from_rethinkdb(self, options):
        ...
        for post in posts.order_by(r.asc("when")).run(conn):
            ...
            dict_row["pub_time"] = datetime.strptime(post["when"], "%Y-%m-%d %H:%M:%S").astimezone(self.europe_prague)
            # dict_row["mod_time"] = ...
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'Successfully imported/updated RethinkDB to Django and it takes {time.seconds//60} minutes and {time.seconds%60} seconds.'))
